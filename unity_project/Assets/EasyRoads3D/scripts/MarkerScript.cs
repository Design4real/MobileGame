...
public bool OCDDOQOCOC;
public bool ODQDOQOO;
public float OOCQQQQQOO;
public float ODOOQQOO;
public Transform[] OODOCOOCOCs;
public float[] trperc;
...
public string OQOQOCODOQ = "0";
public string OCDCQQQDOQ = "0";
...
if(!objectScript.OCCQDCDOQD){  

Vector3 change = transform.position - oldPos;
if(OCDDOQOCOC && oldPos != Vector3.zero && change != Vector3.zero){
int i = 0;
foreach(Transform tr in OODOCOOCOCs){
...
changed = true;
if(objectScript.OCCQDCDOQD){
objectScript.OOCCODDDDQ.specialRoadMaterial = true;
}
}
...
if(objectScript.OOCCODDDDQ == null){
objectScript.OOQCOOCCQO(null, null, null);
}
}
...
for(int i = 0; i < objectScript.OODOCOOCOCs.Length; i++){
if(objectScript.OODOCOOCOCs[i] == this.gameObject)return true;
}
return false;
}
}
