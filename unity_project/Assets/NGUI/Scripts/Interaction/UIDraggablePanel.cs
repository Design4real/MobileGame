...
public class UIDraggablePanel : IgnoreTimeScale
{
	...
	int currentIndex = 0;
	/// <summary> контейнер со страницами </summary>
	Transform grid;
	/// <summary> позиция, которую нужно принять </summary>
	Vector3 currentPos;
	
	public int index{ get{ return currentIndex - 1; } }
	...
	void Awake ()
	{
		...
		// для функционала смещения постранично
		if( dragEffect == DragEffect.MomentumAndPosition || dragEffect == DragEffect.MomentumAndPositionY )
		{
			//если страниц нет, то переключение на функционал смещения с пружиной на границах
	...
	void Start ()
	{
		...
		if( dragEffect == DragEffect.MomentumAndPosition )
		{
			currentIndex = 1;
			currentPos.x = -grid.GetChild( currentIndex ).localPosition.x*MenuController.scaleX;
			SpringPanel.Begin( mPanel.gameObject, currentPos, 13f );
		}
	}
	...
	public bool RestrictWithinBounds (bool instant)
	{
		...
		{
			if ( !instant && ( dragEffect == DragEffect.MomentumAndSpring || dragEffect == DragEffect.MomentumAndPosition || dragEffect == DragEffect.MomentumAndPositionY ) )
			{
				// непонятно почему, но крайние координаты отличаются на 5 едениц от нужный, вкуривать не стал, просто исправил
				if( dragEffect == DragEffect.MomentumAndPosition )
				{
					constraint.x += constraint.x < 0 ? 5 : -5;
				}
				if( dragEffect == DragEffect.MomentumAndPositionY ) 
				{
					constraint.y += constraint.y < 0 ? 5 : -5;
				}
		...
		if (enabled && NGUITools.GetActive(gameObject))
		{
			...
			else
			{
				if (restrictWithinPanel && mPanel.clipping != UIDrawCall.Clipping.None )
				{
					...
					// реализация функционала постриничного смещения
					else if( dragEffect == DragEffect.MomentumAndPosition )
					{
						// в зависимости от направления касания находим номер следующей страницы
						if( UICamera.currentTouch.totalDelta.x < 0 )
						{
							currentIndex++;
							if( currentIndex > grid.childCount - 2 )
							{
								currentIndex = grid.childCount - 2;
							}
						}
						else if( UICamera.currentTouch.totalDelta.x > 0 )
						{
							currentIndex--;
							if( currentIndex < 1 )
							{
						...
						// получаем позицию и смещаем к ней объекты
						currentPos.x = -grid.GetChild( currentIndex ).localPosition.x*MenuController.scaleX;
						SpringPanel.Begin( mPanel.gameObject, currentPos, 13f );
					}
					else if (dragEffect == DragEffect.MomentumAndPositionY)
					{
						// в зависимости от направления касания находим номер следующей страницы
						if( UICamera.currentTouch.totalDelta.y > 0 )
						{
							currentIndex++;
							if( currentIndex > grid.childCount - 1 )
							{
								currentIndex = grid.childCount - 1;
							}
						}
						else if( UICamera.currentTouch.totalDelta.y < 0 )
						{
							currentIndex--;
							if( currentIndex < 0 )
							{
						...
						// получаем позицию и смещаем к ней объекты
						currentPos.y = -grid.GetChild( currentIndex ).localPosition.y*MenuController.scaleY;
						SpringPanel.Begin( mPanel.gameObject, currentPos, 23f );
					}
				}
		...
		if (enabled && NGUITools.GetActive(gameObject) && mShouldMove)
		{
			...
			if (mPlane.Raycast(ray, out dist))
			{
				...
				if (restrictWithinPanel &&
					...
					dragEffect != DragEffect.MomentumAndSpring && 
					dragEffect != DragEffect.MomentumAndPosition &&
					dragEffect != DragEffect.MomentumAndPositionY)
				{
